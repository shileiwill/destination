...
public class NumberOfIslands {
	...
	void dfs(char[][] grid, int i, int j, int[] area) {
		...
		for (int[] dir : directions) {
			...
			if (x >= 0 && x < grid.length && y >= 0 && y < grid[0].length && grid[x][y] == '1') {
				grid[x][y] = '0'; // 变成0 防止重蹈覆辙
				area[0] = area[0] + 1; // 面积一直在增加
				dfs(grid, x, y, area);
			}
		}
	}
	...
	public int numIslandsUnionFind(char[][] grid) {
		...
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
				if (grid[i][j] == '0') { // 只care 1
					continue;
				}
				int id = i * n + j; // Current point
				for (int[] dir : directions) {
					int x1 = i + dir[0];
					int y1 = j + dir[1];
					int id1 = x1 * n + y1; // Next point(s)
					
					// They are connected, lets check if they are associated in UnionFind
					if (x1 >= 0 && x1 < grid.length && y1 >= 0 && y1 < grid[0].length && grid[x1][y1] == '1') {
						int p1 = uf.find(id);
						int p2 = uf.find(id1);
