...
//		wordLadder.ladderLengthSolutionBetter("lat", "pit", dict);
		System.out.print("abc".substring(0, 1));
	}

    // Another easier way
	// 给定一个字典，问从字符串A能不能到串B。每次可以换一个字符，换了之后的串需要在字典里面。DFS/BFS。
	// 延申是问这个方法要频繁调用，怎么优化。具体是先根据字典建立图(Trie?)，然后搜索时候只要搜索相连节点即可。
