...
/**
 * 444. Check whether the original sequence org can be uniquely reconstructed from the sequences in seqs. The org sequence is a permutation of the integers from 1 to n, with 1 ≤ n ≤ 104. Reconstruction means building a shortest common supersequence of the sequences in seqs (i.e., a shortest sequence so that all sequences in seqs are subsequences of it). Determine whether there is only one sequence that can be reconstructed from seqs and it is the org sequence.
...
public class SequenceReconstruction {
    ...
    public boolean sequenceReconstruction(int[] org, List<List<Integer>> seqs) {
        ...
        while (!queue.isEmpty()) {
            ...
            // org comes to the end, or doesnt match
            if (index == org.length || org[index++] != cur) {
                return false;
            }
            
            // It comes here only if they match, and index++ already
