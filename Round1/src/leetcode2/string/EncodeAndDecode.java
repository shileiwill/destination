...
public class EncodeAndDecode {
	    ...
	    public List<String> decode(String s) {
	        ...
	        int i = 0;
	        while (i < s.length()) {
	            int slash = s.indexOf("/", i); // Search from index i
	            int len = Integer.parseInt(s.substring(i, slash)); // Find the length
	            String str = s.substring(slash + 1, slash + len + 1);
	            res.add(str);
