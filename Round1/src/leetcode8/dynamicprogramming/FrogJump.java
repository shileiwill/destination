...
/**
 * 403. A frog is crossing a river. The river is divided into x units and at each unit there may or may not exist a stone. The frog can jump on a stone, but it must not jump into the water.
...
public class FrogJump {
    public boolean canCrossBruteForce(int[] stones) {
        ...
        // It can just only 1 step from unit 1
        return helper(stones, 1, 1);
    }
    
    boolean helper(int[] stones, int unit, int step) {
        if (unit == stones.length - 1) {
            return true;
        }
        
        for (int i = unit + 1; i < stones.length; i++) {
            int requiredSteps = stones[i] - stones[unit];
            if (step >= requiredSteps - 1 && step <= requiredSteps + 1) {
                if (helper(stones, i, requiredSteps)) {
                    return true;
                }
            }
        }
...
On each step, we look if any other stone can be reached from it, if so, we update that stone's steps by adding step, step + 1, step - 1. If we can reach the final stone, we return true. No need to calculate to the last stone.
    */
    public boolean canCross(int[] stones) {
        ...
        // Create empty HashSet for each item
        for (int stone : stones) {
            map.put(Integer.valueOf(stone), new HashSet<Integer>());
        }
