 ...
 * Implement a memcache which support the following features:
 ...
 Actually, a real memcache server will evict keys if memory is not sufficient, and it also supports variety of value types like string and integer. 
...
class Resource {
    public int value;
    public int expiredAt; // Expire at that point, not including
    public Resource(int value, int expiredAt) {
        this.value = value;
...
public class Memcache {
    ...
    public int get(int curtTime, int key) {
        ...
        if (res.expiredAt >= curtTime || res.expiredAt == -1) { // Evict when get
        	return res.value;
        } else {
        	return Integer.MAX_VALUE;
        }
    ...
    public int incr(int curtTime, int key, int delta) {
        if (get(curtTime, key) == Integer.MAX_VALUE) // call get()
            return Integer.MAX_VALUE;
