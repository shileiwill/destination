...
public class MiniUber {
    ...
    Map<Integer, Location> driver2Location = null; // Only the drivers which are on a trip yet
    ...
    // return matched trip information if there have matched rider or null Driver不会主动创建Trip
    public Trip report(int driver_id, double lat, double lng) {
        // Write your code here
        if (driver2Trip.containsKey(driver_id)) {
        	return driver2Trip.get(driver_id); // This driver is on a trip
        }

        if (driver2Location.containsKey(driver_id)) { // Update driver location
    ...
    // @param lat, lng driver's location
    // return a trip 会创建Trip
    public Trip request(int rider_id, double lat, double lng) {
        Trip trip = new Trip(rider_id, lat, lng);
        double distance = -1;
        int driver_id = -1;
        
        for (Map.Entry<Integer, Location> entry : driver2Location.entrySet()) { // 所有的Available的drivers
        ...
        trip.driver_id = driver_id; // 给Trip加上Driver
        driver2Trip.put(driver_id, trip); // Driver不available了
