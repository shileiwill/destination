...
public class RateLimiter {
    ...
    public int count_events(List<Integer> list, int start_timestamp) {
        ...
        if (list.get(right) < start_timestamp) {
        	return 0;
        }
        ...
        while (left <= right) {
            ...
            if (list.get(mid) >= start_timestamp) {
                index = mid;
                right = mid - 1;
            } else {
            	left = mid + 1;
