...
class WebCrawler extends Thread {
	/**
	 ...
	 The capacity, if unspecified, is equal to Integer.MAX_VALUE. Linked nodes are dynamically created upon each insertion unless this would 
	 bring the queue above capacity.
	 
	 java.util.concurrent.BlockingQueue implementations are thread-safe. All queuing methods achieve their effects atomically using internal locks 
	 or other forms of concurrency control. However, the bulk Collection operations addAll, containsAll, retainAll and removeAll 
	 are not necessarily performed atomically unless specified otherwise in an implementation. So it is possible, 
	 for example, for addAll(c) to fail (throwing an exception) after adding only some of the elements in c
	 */
    ...
    public void run() {
        while (true) {
            ...
            try {
                ...
                url = queue.take(); // what if queue is empty? it will wait, then when the thread ends? Queue里暂时没有货了，但是一会儿别的线程可能会produce一些
            } catch (Exception e) {
                // e.printStackTrace(); 
            ...
            if (!visited.contains(url) && domain.endsWith("wikipedia.org")) { // not visited yet, and we care only this domain name
                visited.add(url);
                res.add(url);
